...
#ifndef STRUTTUREDATI_LINKED_LIST_H
#define STRUTTUREDATI_LINKED_LIST_H
...
bool Linked_list<T>::operator==(const Linked_list<T> &lista) const {
    if (this->size() == lista.size()) {
        position p, q;
        for (p = lista.begin(), q = this->begin();
             !(this->end(q)) && !(lista.end(p)); p = lista.next(p), q = this->next(q)) {
...
#endif //STRUTTUREDATI_LINKED_LIST_H
